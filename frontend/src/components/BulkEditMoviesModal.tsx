...
export function BulkEditMoviesModal({ movieIds, onClose, onSave }: Props) {
 ...
 const handleSave = async () => {
  ...
  try {
   ...
   if (minimumAvailability) updates.minimum_availability = minimumAvailability;
   ...
   // Update all selected movies
   await Promise.all(
    movieIds.map(id => api.updateMovie(id, updates))
   );
   
   onSave();
   onClose();
  } catch (error) {
   console.error('Failed to bulk update movies:', error);
  } finally {
   setSaving(false);
  }
 };
   ...
   <div className="bg-white dark:bg-gray-800 shadow-xl w-full max-w-lg mx-4">
    ...
    <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
     <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
      Edit {movieIds.length} Movie{movieIds.length > 1 ? 's' : ''}
     </h2>
     <button
      onClick={onClose}
    ...
    <div className="p-4 space-y-4">
     <p className="text-sm text-gray-500 dark:text-gray-400">
      Only changed fields will be applied to all selected movies.
     </p>
