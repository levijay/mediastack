...
exports.CustomFormatModel = exports.RESOLUTION_VALUES = exports.SOURCE_VALUES = void 0;
...
class CustomFormatModel {
    static create(data) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, data.name, data.media_type || 'both', data.trash_id || null, data.include_when_renaming ? 1 : 0, JSON.stringify(data.specifications));
        return this.findById(id);
    }
    static findById(id) {
        const stmt = database_1.default.prepare('SELECT * FROM custom_formats WHERE id = ?');
        const row = stmt.get(id);
        if (!row)
            return undefined;
        return this.mapRow(row);
    }
    static findByTrashId(trashId, mediaType) {
        if (mediaType) {
            const stmt = database_1.default.prepare('SELECT * FROM custom_formats WHERE trash_id = ? AND media_type = ?');
            const row = stmt.get(trashId, mediaType);
            if (row)
                return this.mapRow(row);
        }
        // Fall back to any media type
        const stmt = database_1.default.prepare('SELECT * FROM custom_formats WHERE trash_id = ?');
        const row = stmt.get(trashId);
        if (!row)
            return undefined;
        return this.mapRow(row);
    }
    static findByName(name, mediaType) {
        if (mediaType) {
            const stmt = database_1.default.prepare('SELECT * FROM custom_formats WHERE name = ? AND media_type = ?');
            const row = stmt.get(name, mediaType);
            if (row)
                return this.mapRow(row);
        }
        // Fall back to any media type
        const stmt = database_1.default.prepare('SELECT * FROM custom_formats WHERE name = ?');
    ...
    static findAll() {
        const stmt = database_1.default.prepare('SELECT * FROM custom_formats ORDER BY name ASC');
        const rows = stmt.all();
        return rows.map(this.mapRow);
    }
    static findByMediaType(mediaType) {
        const stmt = database_1.default.prepare('SELECT * FROM custom_formats WHERE media_type = ? OR media_type = ? ORDER BY name ASC');
        const rows = stmt.all(mediaType, 'both');
        return rows.map(this.mapRow);
    }
    static update(id, data) {
        ...
        if (updates.length > 0) {
            ...
            values.push(id);
            const stmt = database_1.default.prepare(`UPDATE custom_formats SET ${updates.join(', ')} WHERE id = ?`);
            stmt.run(...values);
        }
        return this.findById(id);
    }
    static delete(id) {
        const stmt = database_1.default.prepare('DELETE FROM custom_formats WHERE id = ?');
        const result = stmt.run(id);
        return result.changes > 0;
    }
    static count() {
        const stmt = database_1.default.prepare('SELECT COUNT(*) as count FROM custom_formats');
        return stmt.get().count;
    }
    // Import from Trash Guides JSON format
    static importFromTrashGuides(json, mediaType = 'both') {
    ...
    static setProfileScore(profileId, customFormatId, score) {
        const stmt = database_1.default.prepare(`
    ...
    static removeProfileScore(profileId, customFormatId) {
        const stmt = database_1.default.prepare(`
    ...
    static clearProfileScores(profileId) {
        const stmt = database_1.default.prepare('DELETE FROM quality_profile_custom_formats WHERE profile_id = ?');
        return stmt.run(profileId).changes;
    }
    // Calculate custom format score for a release
    static calculateReleaseScore(releaseTitle, profileId, releaseSize) {
    ...
    static matchesFormat(releaseTitle, specifications, releaseSize) {
    ...
    static matchesSpecification(releaseTitle, spec, releaseSize) {
        switch (spec.implementation) {
            case 'ReleaseTitleSpecification':
                try {
                    const regex = new RegExp(spec.fields.value, 'i');
            ...
            case 'SourceSpecification': {
                const sourceValue = spec.fields.value;
                const sourceName = exports.SOURCE_VALUES[sourceValue]?.toLowerCase();
                if (!sourceName)
                    return false;
            ...
            case 'ResolutionSpecification': {
                const resValue = spec.fields.value;
            ...
            case 'SizeSpecification': {
                if (releaseSize === undefined)
                    return true; // Can't evaluate without size
            ...
            case 'LanguageSpecification': {
                ...
                const langValue = spec.fields.value;
                const titleLower = releaseTitle.toLowerCase();
            ...
            case 'IndexerFlagSpecification': {
                ...
                const titleLower = releaseTitle.toLowerCase();
                const flagValue = spec.fields.value;
            ...
            case 'QualityModifierSpecification': {
                const modValue = spec.fields.value;
                const titleUpper = releaseTitle.toUpperCase();
                // Common modifiers
                switch (modValue) {
                    case 1: // REMUX
...
exports.CustomFormatModel = CustomFormatModel;
