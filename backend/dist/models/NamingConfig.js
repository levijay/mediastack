...
exports.NamingConfigModel = void 0;
const database_1 = __importDefault(require("../config/database"));
class NamingConfigModel {
    // Get the naming configuration
    static get() {
        const stmt = database_1.default.prepare('SELECT * FROM naming_config WHERE id = 1');
        const row = stmt.get();
        if (!row) {
            // Create default if doesn't exist
            database_1.default.prepare('INSERT INTO naming_config (id) VALUES (1)').run();
            return this.get();
        }
        return {
            ...row,
    ...
    static update(data) {
        ...
        if (updates.length > 0) {
            ...
            const stmt = database_1.default.prepare(`UPDATE naming_config SET ${updates.join(', ')} WHERE id = 1`);
            stmt.run(...values);
        }
        return this.get();
    }
...
exports.NamingConfigModel = NamingConfigModel;
//# sourceMappingURL=NamingConfig.js.map