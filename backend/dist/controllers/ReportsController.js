...
class ReportsController {
    ...
    static async getFilterOptions(req, res) {
        try {
            ...
            const movieQualities = database_1.default.prepare(`
      ...
      `).all();
            ...
            const episodeQualities = database_1.default.prepare(`
      ...
      `).all();
            ...
            const resolutions = database_1.default.prepare(`
      ...
      `).all();
            ...
            const hdrTypes = database_1.default.prepare(`
      ...
      `).all();
            ...
            const audioChannels = database_1.default.prepare(`
      ...
      `).all();
            ...
            const releaseGroups = database_1.default.prepare(`
    ...
    static async searchMovies(req, res) {
        try {
            ...
            const orderBy = sortColumns[sortBy] || 'total_size';
        ...
        `;
            ...
            const results = database_1.default.prepare(query).all(...params);
            ...
            if (titleMismatch === 'true') {
                processedResults = processedResults.filter(movie => {
                    ...
                    return movie.files.some((f) => {
                        ...
                        const matchingWords = movieWords.filter(word => fileWords.some(fw => fw.includes(word) || word.includes(fw)));
            ...
            logger_1.default.error('Search movies error:', error);
     ...
     */
    static async searchEpisodes(req, res) {
        try {
            const { quality, videoCodec, audioCodec, sizeFrom, sizeTo, hasFile, monitored, sortBy = 'file_size', sortDir = 'desc', limit = 100 } = req.query;
            let conditions = ['e.has_file = 1']; // Only episodes with files
            let params = [];
            if (quality) {
            ...
            const orderBy = sortColumns[sortBy] || 'e.file_size';
            const orderDir = sortDir === 'asc' ? 'ASC' : 'DESC';
            const query = `
        SELECT 
      ...
      `;
            ...
            logger_1.default.error('Search episodes error:', error);
    ...
    static async getStats(req, res) {
        try {
            // Movie stats
            const movieStats = database_1.default.prepare(`
        SELECT 
        ...
        FROM movies
      `).get();
            const movieFileStats = database_1.default.prepare(`
        SELECT 
      ...
      `).get();
            // Movies with multiple files
            const multipleFilesCount = database_1.default.prepare(`
      ...
      `).get();
            // Series stats
            const seriesStats = database_1.default.prepare(`
        SELECT 
          COUNT(*) as total_series
        FROM tv_series
      `).get();
            const episodeStats = database_1.default.prepare(`
        SELECT 
