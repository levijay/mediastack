...
class MediaInfoService {
    ...
    async getMediaInfo(filePath) {
        ...
        try {
            ...
            if (!ffprobePath) {
                logger_1.default.warn('[MediaInfo] ffprobe not found, using filename parsing only');
                return filenameInfo;
            }
            ...
            if (videoStreams.length > 0) {
                ...
                if (!info.resolution && video.height) {
                    if (video.height >= 2000)
                        info.resolution = '2160p';
                    else if (video.height >= 1000)
                        info.resolution = '1080p';
                    else if (video.height >= 700)
                        info.resolution = '720p';
                    else if (video.height >= 400)
                        info.resolution = '480p';
                    else
                        info.resolution = `${video.height}p`;
                }
            ...
            logger_1.default.debug(`[MediaInfo] Extracted info for ${path_1.default.basename(filePath)}:`, info);
            return info;
        }
        catch (error) {
            logger_1.default.error(`[MediaInfo] Error extracting info for ${filePath}:`, error);
     ...
     */
    async runFFprobe(ffprobePath, filePath) {
        try {
            const { stdout } = await execAsync(`${ffprobePath} -v quiet -print_format json -show_format -show_streams "${filePath}"`, { maxBuffer: 10 * 1024 * 1024 } // 10MB buffer
            );
            return JSON.parse(stdout);
        }
        catch (error) {
            logger_1.default.error(`[MediaInfo] FFprobe failed for ${filePath}:`, error);
            return null;
        }
    }
    /**
    ...
    detectHDR(video) {
        ...
        if (video.side_data_list?.some(sd => sd.side_data_type === 'DOVI configuration record' ||
        ...
        if (isPQ && isBT2020) {
            ...
            if (video.side_data_list?.some(sd => sd.side_data_type === 'Mastering display metadata' ||
     ...
     */
    normalizeAudioCodec(codec, longName) {
        const codecLower = codec.toLowerCase();
        const longNameLower = (longName || '').toLowerCase();
        // DTS variants
        if (longNameLower.includes('dts-hd ma') || longNameLower.includes('dts-hd master'))
            return 'DTS-HD MA';
        if (longNameLower.includes('dts:x') || longNameLower.includes('dts-x'))
            return 'DTS-X';
        if (longNameLower.includes('dts-hd'))
            return 'DTS-HD';
        if (codecLower === 'dts' || codecLower === 'dca')
            return 'DTS';
        // Dolby variants
        if (longNameLower.includes('truehd') || codecLower === 'truehd') {
            if (longNameLower.includes('atmos'))
                return 'TrueHD Atmos';
            return 'TrueHD';
        }
        ...
        if (codecLower === 'ac3' || codecLower === 'a_ac3')
            return 'DD';
        // Other common codecs
        if (codecLower === 'aac')
            return 'AAC';
        ...
        if (codecLower === 'opus')
            return 'Opus';
        if (codecLower === 'mp3' || codecLower === 'mp3float')
            return 'MP3';
        if (codecLower === 'pcm_s16le' || codecLower === 'pcm_s24le')
            return 'PCM';
     ...
     */
    formatChannels(channels, layout) {
        if (!channels)
            return '';
        ...
        if (layout) {
            ...
            if (layout.includes('5.1'))
                return '5.1';
            if (layout.includes('stereo'))
                return '2.0';
            if (layout.includes('mono'))
                return '1.0';
        }
    ...
    parseFromFilename(filePath) {
        ...
        // Video codec
        if (filename.match(/x265|HEVC|H\.?265/i))
            info.videoCodec = 'x265';
        else if (filename.match(/x264|AVC|H\.?264/i))
            info.videoCodec = 'x264';
        else if (filename.match(/AV1/i))
            info.videoCodec = 'AV1';
        else if (filename.match(/VP9/i))
            info.videoCodec = 'VP9';
        // HDR
        ...
        // Audio codec
        if (filename.match(/DTS[-\.]?X/i))
            info.audioCodec = 'DTS-X';
        else if (filename.match(/DTS[-\.]?HD[-\.]?MA/i))
            info.audioCodec = 'DTS-HD MA';
        else if (filename.match(/TrueHD[-\.]?Atmos|Atmos/i))
            info.audioCodec = 'TrueHD Atmos';
        else if (filename.match(/TrueHD/i))
            info.audioCodec = 'TrueHD';
        else if (filename.match(/DD\+|DDP|EAC3/i))
            info.audioCodec = 'DD+';
        else if (filename.match(/DD5\.1|AC3/i))
            info.audioCodec = 'DD';
        else if (filename.match(/DTS/i))
            info.audioCodec = 'DTS';
        else if (filename.match(/FLAC/i))
        ...
        else if (filename.match(/5\.1/i))
            info.audioChannels = '5.1';
        else if (filename.match(/2\.0/i))
     ...
     */
    detectQualitySourceFromBitrate(bitrate, resolution) {
        // Bitrate in kbps
...
exports.mediaInfoService = new MediaInfoService();
