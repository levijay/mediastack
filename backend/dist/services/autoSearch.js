...
exports.autoSearchService = exports.AutoSearchService = void 0;
...
class AutoSearchService {
     ...
     */
    async searchAndDownloadMovie(movieId, forceUpgradeSearch = false) {
        try {
            ...
            if (movie.has_file && movie.quality_profile_id) {
                ...
                if (!currentQuality || currentQuality === 'Unknown') {
                    const fileQualityStmt = database_1.default.prepare('SELECT quality, is_proper, is_repack FROM movie_files WHERE movie_id = ? LIMIT 1');
                    const currentFile = fileQualityStmt.get(movie.id);
                    if (currentFile) {
                    ...
                    const fileStmt = database_1.default.prepare('SELECT is_proper, is_repack FROM movie_files WHERE movie_id = ? LIMIT 1');
                    const currentFile = fileStmt.get(movie.id);
                    if (currentFile) {
                        currentIsProper = Boolean(currentFile.is_proper);
                        currentIsRepack = Boolean(currentFile.is_repack);
                    }
                ...
                if (!forceUpgradeSearch && !profile?.upgrade_allowed) {
                    logger_1.default.info(`Skipping upgrade search for ${movie.title} - upgrades not allowed`);
                    return null;
                }
                if (QualityProfile_1.QualityProfileModel.meetsCutoff(movie.quality_profile_id, currentQuality) && !forceUpgradeSearch) {
                    logger_1.default.info(`Skipping upgrade search for ${movie.title} - already at cutoff quality (${currentQuality})`);
                    return null;
                }
                logger_1.default.info(`Searching for upgrade for movie: ${movie.title} (current: ${currentQuality}${currentIsProper ? ' PROPER' : ''}${currentIsRepack ? ' REPACK' : ''})`);
            }
            else {
                logger_1.default.info(`Auto-searching for movie: ${movie.title} (${movie.year})`);
            }
            // Search all indexers
            const releases = await indexer_1.indexerService.searchMovie(movie.title, movie.year ?? undefined);
            if (releases.length === 0) {
                logger_1.default.warn(`No releases found for: ${movie.title}`);
                return null;
            }
            // Get blacklisted releases for this movie
            const blacklistedReleases = ReleaseBlacklist_1.ReleaseBlacklistModel.getForMovie(movieId);
            const blacklistedTitles = blacklistedReleases.map(r => r.release_title);
            if (blacklistedTitles.length > 0) {
                logger_1.default.info(`[Blacklist] ${blacklistedTitles.length} releases blacklisted for ${movie.title}`);
            }
            ...
            if (scored.length === 0) {
                logger_1.default.warn(`No suitable releases found for: ${movie.title}`);
                return null;
            }
            // Select best release
            const best = scored[0];
            // Check if best release is proper/repack
            const { isProper: newIsProper, isRepack: newIsRepack } = QualityProfile_1.QualityProfileModel.isProperOrRepack(best.title);
            ...
            if (movie.has_file) {
                ...
                if (!currentQualityForUpgrade || currentQualityForUpgrade === 'Unknown') {
                    const fileQualityStmt = database_1.default.prepare('SELECT quality FROM movie_files WHERE movie_id = ? LIMIT 1');
                    const currentFile = fileQualityStmt.get(movie.id);
                    currentQualityForUpgrade = currentFile?.quality || 'Unknown';
                }
                if (!QualityProfile_1.QualityProfileModel.shouldUpgrade(movie.quality_profile_id, currentQualityForUpgrade, best.quality, {
                    currentIsProper,
                    currentIsRepack,
                    newIsProper,
                    newIsRepack
                })) {
                    logger_1.default.info(`No quality upgrade available for ${movie.title} (current: ${currentQualityForUpgrade}, best: ${best.quality})`);
                    return null;
                }
                logger_1.default.info(`Found quality upgrade for ${movie.title}: ${currentQualityForUpgrade} -> ${best.quality}${newIsProper ? ' PROPER' : ''}${newIsRepack ? ' REPACK' : ''}`);
            }
            logger_1.default.info(`Selected best release: ${best.title} (score: ${best.score}, protocol: ${best.protocol || 'unknown'})`);
            // Start download
            ...
            const addResult = await downloadClient_1.downloadClientService.addDownload(best.downloadUrl, 'movie', savePath, undefined, // Let service pick the client
            best.protocol);
            if (addResult.success) {
                ...
                logger_1.default.info(`Download started for: ${movie.title}`);
                return best;
            }
            else {
                logger_1.default.error(`Failed to add download: ${addResult.message}`);
                Download_1.DownloadModel.updateStatus(download.id, 'failed');
                return { ...best, error: addResult.message };
            }
        }
        catch (error) {
            logger_1.default.error('Auto-search error:', error);
            return null;
        }
    }
    /**
     ...
     */
    async searchAndDownloadEpisode(seriesId, seasonNumber, episodeNumber, forceUpgradeSearch = false) {
        try {
            ...
            // Get quality profile
            const profileStmt = database_1.default.prepare('SELECT * FROM quality_profiles WHERE id = ?');
            const profile = profileStmt.get(series.quality_profile_id);
            if (profile) {
                profile.upgrade_allowed = Boolean(profile.upgrade_allowed);
                profile.items = JSON.parse(profile.items || '[]');
            }
            ...
            if (episode.has_file && series.quality_profile_id) {
                ...
                if (!forceUpgradeSearch && !profile?.upgrade_allowed) {
                    logger_1.default.debug(`Skipping upgrade search for ${series.title} S${seasonNumber}E${episodeNumber} - upgrades not allowed`);
                    return null;
                }
                if (QualityProfile_1.QualityProfileModel.meetsCutoff(series.quality_profile_id, currentQuality) && !forceUpgradeSearch) {
                    logger_1.default.debug(`Skipping upgrade search for ${series.title} S${seasonNumber}E${episodeNumber} - already at cutoff quality (${currentQuality})`);
                    return null;
                }
                logger_1.default.info(`Searching for upgrade for: ${series.title} S${seasonNumber}E${episodeNumber} (current: ${currentQuality}${currentIsProper ? ' PROPER' : ''}${currentIsRepack ? ' REPACK' : ''})`);
            }
            else {
                logger_1.default.info(`Auto-searching for: ${series.title} S${seasonNumber}E${episodeNumber}`);
            }
            // Search all indexers
            const releases = await indexer_1.indexerService.searchTV(series.title, seasonNumber, episodeNumber);
            if (releases.length === 0) {
                logger_1.default.warn(`No releases found for: ${series.title} S${seasonNumber}E${episodeNumber}`);
                return null;
            }
            // Get blacklisted releases for this episode
            const blacklistedReleases = ReleaseBlacklist_1.ReleaseBlacklistModel.getForSeries(seriesId)
                .filter(r => !r.season_number || r.season_number === seasonNumber)
                .filter(r => !r.episode_number || r.episode_number === episodeNumber);
            const blacklistedTitles = blacklistedReleases.map(r => r.release_title);
            if (blacklistedTitles.length > 0) {
                logger_1.default.info(`[Blacklist] ${blacklistedTitles.length} releases blacklisted for ${series.title} S${seasonNumber}E${episodeNumber}`);
            }
            // Score and filter releases with title validation and blacklist
            const scored = this.scoreReleases(releases, profile, series.title, blacklistedTitles);
            if (scored.length === 0) {
                logger_1.default.warn(`No suitable releases found`);
                return null;
            }
            // Select best release
            const best = scored[0];
            ...
            if (episode.has_file) {
                const currentQuality = episode.quality || 'Unknown';
                if (!QualityProfile_1.QualityProfileModel.shouldUpgrade(series.quality_profile_id, currentQuality, best.quality, {
                    currentIsProper,
                    currentIsRepack,
                    newIsProper,
                    newIsRepack
                })) {
                    logger_1.default.info(`No quality upgrade available for ${series.title} S${seasonNumber}E${episodeNumber} (current: ${currentQuality}, best: ${best.quality})`);
                    return null;
                }
                logger_1.default.info(`Found quality upgrade for ${series.title} S${seasonNumber}E${episodeNumber}: ${currentQuality} -> ${best.quality}${newIsProper ? ' PROPER' : ''}${newIsRepack ? ' REPACK' : ''}`);
            }
            logger_1.default.info(`Selected best release: ${best.title} (score: ${best.score}, protocol: ${best.protocol || 'unknown'})`);
            // Start download
            const download = Download_1.DownloadModel.create({
            ...
            const addResult = await downloadClient_1.downloadClientService.addDownload(best.downloadUrl, 'tv', savePath, undefined, // Let service pick the client
            best.protocol);
            if (addResult.success) {
                if (addResult.downloadId) {
                    Download_1.DownloadModel.updateTorrentHash(download.id, addResult.downloadId);
                ...
                logger_1.default.info(`Download started for: ${series.title} S${seasonNumber}E${episodeNumber}`);
                return best;
            }
            else {
                logger_1.default.error(`Failed to add download: ${addResult.message}`);
     ...
     */
    async searchAndDownloadSeries(seriesId, includeUpgrades = true) {
        try {
            const series = TVSeries_1.TVSeriesModel.findById(seriesId);
            if (!series) {
                logger_1.default.error(`Series not found: ${seriesId}`);
            ...
            logger_1.default.info(`Auto-searching for ${series.title} (missing episodes${upgradesAllowed ? ' + upgrades' : ''})`);
            ...
            for (const season of seasons) {
                ...
                if (season.season_number === 0)
                    continue;
                const episodes = TVSeries_1.TVSeriesModel.findEpisodesBySeason(seriesId, season.season_number);
                for (const episode of episodes) {
                    ...
                    if (!isMissing && !shouldSearchForUpgrade)
                        continue;
                    searched++;
                    try {
                        const result = await this.searchAndDownloadEpisode(seriesId, season.season_number, episode.episode_number);
                        if (result) {
                            found++;
                        }
                        ...
                        logger_1.default.error(`Failed to search for S${season.season_number}E${episode.episode_number}:`, err);
                    }
                }
            }
            logger_1.default.info(`Finished auto-search for ${series.title}: searched ${searched}, found ${found}`);
            return { searched, found };
        }
        catch (error) {
            logger_1.default.error('Auto-search series error:', error);
            return { searched: 0, found: 0 };
        }
    }
    /**
     ...
     */
    scoreReleases(releases, profile, expectedTitle, blacklistedTitles = [], expectedYear) {
        ...
        const normalizedBlacklist = new Set(blacklistedTitles.map(t => t.toLowerCase().trim()));
        return releases
            .filter(r => {
            // Filter out blacklisted releases
            const normalizedTitle = r.title?.toLowerCase().trim();
            if (normalizedBlacklist.has(normalizedTitle)) {
                logger_1.default.debug(`[SCORE] Rejected - blacklisted release: "${r.title}"`);
                return false;
            }
            return true;
        })
            ...
            const customFormatScore = profile.id
                ? CustomFormat_1.CustomFormatModel.calculateReleaseScore(r.title, profile.id, r.size)
                : 0;
            return {
                ...r,
                quality,
            ...
            if (r.customFormatScore < minCfScore) {
                logger_1.default.debug(`[SCORE] Rejected - custom format score ${r.customFormatScore} below minimum ${minCfScore}: "${r.title}"`);
                return false;
            }
            return true;
        })
     ...
     */
    calculateScore(release, profile, expectedTitle, expectedYear) {
        let score = 0;
        ...
        if (expectedTitle) {
            ...
            if (!titleMatch) {
                logger_1.default.debug(`[SCORE] Rejected - title mismatch: "${release.title}" vs "${expectedTitle}"`);
        ...
        if (expectedYear) {
            ...
            if (releaseYear && Math.abs(releaseYear - expectedYear) > 1) {
                ...
                logger_1.default.debug(`[SCORE] Rejected - year mismatch: release has ${releaseYear}, expected ${expectedYear}: "${release.title}"`);
                return 0;
            }
        }
        const quality = this.detectQuality(release.title);
        // Check if quality is allowed in profile using QualityProfileModel
        if (!QualityProfile_1.QualityProfileModel.meetsProfile(profile.id, quality)) {
            logger_1.default.debug(`[SCORE] Rejected - quality ${quality} not allowed in profile ${profile.name}`);
            return 0;
        }
        // Quality is allowed, add base score
        score += 100;
        // Calculate quality ranks for comparison
        const qualityWeight = QualityProfile_1.QualityProfileModel.getQualityWeight(quality);
        const cutoffWeight = QualityProfile_1.QualityProfileModel.getQualityWeight(profile.cutoff_quality);
     ...
     */
    validateTitleMatch(releaseTitle, expectedTitle) {
        ...
        const matchedWords = expectedWords.filter(expectedWord => releaseWords.some(releaseWord => releaseWord === expectedWord));
        ...
        if (matchRatio < requiredRatio) {
            logger_1.default.debug(`[VALIDATE] Title mismatch (${Math.round(matchRatio * 100)}% match): "${extractedTitle}" vs "${expectedTitle}"`);
            return false;
        }
        if (unmatchedReleaseWords.length > maxExtraWords) {
            logger_1.default.debug(`[VALIDATE] Too many extra words (${unmatchedReleaseWords.length} > ${maxExtraWords}): "${extractedTitle}" vs "${expectedTitle}"`);
            return false;
        }
        return true;
    }
     ...
     */
    extractYearFromRelease(releaseName) {
     ...
     */
    async searchAllMissingMovies(concurrentLimit = 1) {
        try {
            // Get all monitored movies without files
            const allMissingMovies = Movie_1.MovieModel.findMissing();
            // Get only those that are actually available
            const missingMovies = Movie_1.MovieModel.findMissingAndAvailable();
            const skippedCount = allMissingMovies.length - missingMovies.length;
            if (missingMovies.length === 0) {
                if (skippedCount > 0) {
                    logger_1.default.info(`No missing movies available for search. ${skippedCount} movie(s) skipped due to minimum availability settings.`);
                }
                else {
                    logger_1.default.info('No missing monitored movies to search');
                }
                return { total: allMissingMovies.length, searched: 0, found: 0, skipped: skippedCount };
            }
            logger_1.default.info(`Starting missing movie search: ${missingMovies.length} movies to search, ${skippedCount} skipped (not yet available)`);
            let searched = 0;
            let found = 0;
            ...
            for (let i = 0; i < missingMovies.length; i += concurrentLimit) {
                ...
                const results = await Promise.all(batch.map(async (movie) => {
                    try {
                        ...
                        logger_1.default.error(`Failed to search for movie ${movie.title}:`, err);
                        return 0;
                    }
                }));
                ...
                // Log progress
                logger_1.default.info(`Missing movie search progress: ${searched}/${missingMovies.length} searched, ${found} found`);
                // Delay between batches to avoid hammering indexers
                if (i + concurrentLimit < missingMovies.length) {
                    await new Promise(resolve => setTimeout(resolve, 3000));
                }
            }
            logger_1.default.info(`Missing movie search complete: ${searched} searched, ${found} found, ${skippedCount} skipped`);
            return { total: allMissingMovies.length, searched, found, skipped: skippedCount };
        }
        catch (error) {
            logger_1.default.error('Search all missing movies error:', error);
     ...
     */
    async searchAllMissingSeries(concurrentLimit = 1) {
        try {
            // Get all monitored series
            const monitoredSeries = TVSeries_1.TVSeriesModel.findMonitored();
            if (monitoredSeries.length === 0) {
                logger_1.default.info('No monitored series to search');
            ...
            for (const series of monitoredSeries) {
                const seasons = TVSeries_1.TVSeriesModel.findSeasonsBySeriesId(series.id);
                for (const season of seasons) {
                    // Skip specials (season 0)
                    if (season.season_number === 0)
                        continue;
                    const episodes = TVSeries_1.TVSeriesModel.findEpisodesBySeason(series.id, season.season_number);
                    for (const episode of episodes) {
            ...
            if (allMissingEpisodes.length === 0) {
                logger_1.default.info('No missing monitored episodes to search');
                return { total: 0, searched: 0, found: 0 };
            }
            logger_1.default.info(`Starting missing episode search: ${allMissingEpisodes.length} episodes to search`);
            let searched = 0;
            let found = 0;
            // Process in batches based on concurrent limit
            for (let i = 0; i < allMissingEpisodes.length; i += concurrentLimit) {
                ...
                const results = await Promise.all(batch.map(async (ep) => {
                    try {
                        ...
                        logger_1.default.error(`Failed to search for ${ep.seriesTitle} S${ep.season}E${ep.episode}:`, err);
                        return 0;
                    }
                }));
                searched += batch.length;
                ...
                if ((i / concurrentLimit) % 10 === 0 || i + concurrentLimit >= allMissingEpisodes.length) {
                    logger_1.default.info(`Missing episode search progress: ${searched}/${allMissingEpisodes.length} searched, ${found} found`);
                }
                // Delay between batches to avoid hammering indexers
            ...
            logger_1.default.info(`Missing episode search complete: ${searched} searched, ${found} found`);
            return { total: allMissingEpisodes.length, searched, found };
        }
        catch (error) {
            logger_1.default.error('Search all missing series error:', error);
            return { total: 0, searched: 0, found: 0 };
        }
    }
    /**
     ...
     */
    async searchAllMissing(concurrentLimit = 1) {
        logger_1.default.info('=== Starting Missing Media Search ===');
        // Search movies first
        ...
        logger_1.default.info('=== Missing Media Search Complete ===');
        logger_1.default.info(`Movies: ${movieResults.found}/${movieResults.total} found`);
        logger_1.default.info(`Episodes: ${episodeResults.found}/${episodeResults.total} found`);
        return {
            movies: movieResults,
     ...
     */
    async searchCutoffUnmetMovies(concurrentLimit = 1) {
        try {
            // Get all monitored movies with files
            const moviesWithFiles = Movie_1.MovieModel.findWithFiles();
            // Filter to those below cutoff quality
            const cutoffUnmet = [];
            for (const movie of moviesWithFiles) {
                if (!movie.quality_profile_id || !movie.quality)
                    continue;
                // Check if profile allows upgrades
                const profile = QualityProfile_1.QualityProfileModel.findById(movie.quality_profile_id);
                if (!profile || !profile.upgrade_allowed)
                    continue;
                // Check if current quality is below cutoff
                if (!QualityProfile_1.QualityProfileModel.meetsCutoff(movie.quality_profile_id, movie.quality)) {
                    cutoffUnmet.push(movie);
                }
            }
            if (cutoffUnmet.length === 0) {
                logger_1.default.info('No movies below cutoff quality to search');
                return { total: 0, searched: 0, found: 0 };
            }
            logger_1.default.info(`Starting cutoff unmet movie search: ${cutoffUnmet.length} movies to search`);
            let searched = 0;
            let found = 0;
            // Process in batches based on concurrent limit
            for (let i = 0; i < cutoffUnmet.length; i += concurrentLimit) {
                ...
                const results = await Promise.all(batch.map(async (movie) => {
                    try {
                        ...
                        logger_1.default.error(`Failed to search upgrade for movie ${movie.title}:`, err);
                        return 0;
                    }
                }));
                searched += batch.length;
                found += results.reduce((a, b) => a + b, 0);
                // Log progress
                logger_1.default.info(`Cutoff unmet movie search progress: ${searched}/${cutoffUnmet.length} searched, ${found} found`);
                // Delay between batches to avoid hammering indexers
                if (i + concurrentLimit < cutoffUnmet.length) {
                    await new Promise(resolve => setTimeout(resolve, 3000));
                }
            }
            logger_1.default.info(`Cutoff unmet movie search complete: ${searched} searched, ${found} upgrades found`);
            return { total: cutoffUnmet.length, searched, found };
        }
        catch (error) {
            logger_1.default.error('Search cutoff unmet movies error:', error);
            return { total: 0, searched: 0, found: 0 };
        }
    }
    /**
     ...
     */
    async searchCutoffUnmetSeries(concurrentLimit = 1) {
        try {
            // Get all monitored series
            const monitoredSeries = TVSeries_1.TVSeriesModel.findMonitored();
            if (monitoredSeries.length === 0) {
                logger_1.default.info('No monitored series for cutoff search');
                return { total: 0, searched: 0, found: 0 };
            }
            // Get all episodes with files that are below cutoff
            const cutoffUnmetEpisodes = [];
            for (const series of monitoredSeries) {
                if (!series.quality_profile_id)
                    continue;
                // Check if profile allows upgrades
                ...
                for (const season of seasons) {
                    ...
                    for (const episode of episodes) {
                        ...
                        if (!episode.has_file || !episode.monitored || !episode.quality)
                            continue;
                        // Check if current quality is below cutoff
                        if (!QualityProfile_1.QualityProfileModel.meetsCutoff(series.quality_profile_id, episode.quality)) {
            ...
            if (cutoffUnmetEpisodes.length === 0) {
                logger_1.default.info('No episodes below cutoff quality to search');
                return { total: 0, searched: 0, found: 0 };
            }
            logger_1.default.info(`Starting cutoff unmet episode search: ${cutoffUnmetEpisodes.length} episodes to search`);
            let searched = 0;
            let found = 0;
            // Process in batches based on concurrent limit
            for (let i = 0; i < cutoffUnmetEpisodes.length; i += concurrentLimit) {
                ...
                const results = await Promise.all(batch.map(async (ep) => {
                    try {
                        ...
                        logger_1.default.error(`Failed to search upgrade for ${ep.seriesTitle} S${ep.season}E${ep.episode}:`, err);
                        return 0;
                    }
                }));
                searched += batch.length;
                ...
                if ((i / concurrentLimit) % 10 === 0 || i + concurrentLimit >= cutoffUnmetEpisodes.length) {
                    logger_1.default.info(`Cutoff unmet episode search progress: ${searched}/${cutoffUnmetEpisodes.length} searched, ${found} found`);
                }
                // Delay between batches to avoid hammering indexers
            ...
            logger_1.default.info(`Cutoff unmet episode search complete: ${searched} searched, ${found} upgrades found`);
            return { total: cutoffUnmetEpisodes.length, searched, found };
        }
        catch (error) {
            logger_1.default.error('Search cutoff unmet series error:', error);
            return { total: 0, searched: 0, found: 0 };
        }
    }
    /**
     ...
     * Used by the background worker
     */
    async searchAllCutoffUnmet(concurrentLimit = 1) {
        logger_1.default.info('=== Starting Cutoff Unmet Search (Quality Upgrades) ===');
        // Search movies first
        ...
        logger_1.default.info('=== Cutoff Unmet Search Complete ===');
        logger_1.default.info(`Movies: ${movieResults.found}/${movieResults.total} upgrades found`);
        logger_1.default.info(`Episodes: ${episodeResults.found}/${episodeResults.total} upgrades found`);
        return {
            movies: movieResults,
            episodes: episodeResults
        };
...
exports.autoSearchService = new AutoSearchService();
