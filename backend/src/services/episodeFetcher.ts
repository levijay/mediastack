...
export class EpisodeFetcherService {
  /**
   ...
   */
  async fetchAndPopulateEpisodes(seriesId: string): Promise<number> {
    try {
      ...
      const useTVDB = series.tvdb_id && await tvdbService.isConfigured();
      
      if (useTVDB && series.tvdb_id) {
        logger.info(`Fetching episodes from TVDB for series: ${series.title}`);
        return this.fetchFromTVDB(series);
      } else if (series.tmdb_id) {
        logger.info(`Fetching episodes from TMDB for series: ${series.title}`);
        return this.fetchFromTMDB(series);
      } else {
        logger.error(`Series ${series.title} has no TVDB or TMDB ID`);
        return 0;
      }
    } catch (error) {
   ...
   */
  private async fetchFromTVDB(series: any): Promise<number> {
    try {
      const tvdbData = await tvdbService.getSeriesDetails(series.tvdb_id);
      
      if (!tvdbData || !tvdbData.episodes) {
        logger.warn(`No episodes found from TVDB for series: ${series.title}`);
      ...
      for (const [seasonNumber, episodes] of episodesBySeason) {
        try {
          ...
          const existingSeasons = TVSeriesModel.findSeasonsBySeriesId(series.id);
          ...
          if (!dbSeason) {
            dbSeason = TVSeriesModel.createSeason(series.id, seasonNumber, shouldMonitorSeason);
            logger.info(`Created season ${seasonNumber} for: ${series.title} (monitored: ${shouldMonitorSeason})`);
          }

          // Get existing episodes for this season
          const existingEpisodes = TVSeriesModel.findEpisodesBySeason(series.id, seasonNumber);
          ...
          logger.error(`Failed to process season ${seasonNumber}:`, error);
        }
      }

      logger.info(`Fetched ${totalEpisodes} episodes from TVDB for: ${series.title}`);

      // Auto-scan for episode files
      if (series.folder_path && fs.existsSync(series.folder_path)) {
        await this.scanEpisodeFiles(series.id, series.folder_path, series.title);
      }

      return totalEpisodes;
    } catch (error) {
      logger.error(`TVDB fetch failed for ${series.title}, trying TMDB:`, error);
      if (series.tmdb_id) {
        return this.fetchFromTMDB(series);
      }
      return 0;
   ...
   */
  private async fetchFromTMDB(series: any): Promise<number> {
    if (!series.tmdb_id) {
      logger.error(`Series missing TMDB ID: ${series.title}`);
      return 0;
    }
    ...
    if (!tmdbData.seasons) {
      logger.warn(`No seasons found for series: ${series.title}`);
      return 0;
    }
    ...
    for (const season of sortedSeasons) {
      try {
        ...
        if (!dbSeason) {
          dbSeason = TVSeriesModel.createSeason(series.id, season.season_number, shouldMonitorSeason);
          logger.info(`Created season ${season.season_number} for: ${series.title} (monitored: ${shouldMonitorSeason})`);
        }

        const existingEpisodes = TVSeriesModel.findEpisodesBySeason(series.id, season.season_number);
        if (existingEpisodes.length > 0 && existingEpisodes.length >= (season.episode_count || 0)) {
          continue;
        }

        logger.info(`Fetching season ${season.season_number} details for: ${series.title}`);
        const seasonDetails = await tmdbService.getSeasonDetails(series.tmdb_id, season.season_number);
        ...
        if (monitorNewSeasons === 'current' && season.season_number === currentSeasonNumber && !isEnded) {
          const hasFutureEpisodes = seasonDetails.episodes.some((ep: any) => !ep.air_date || ep.air_date > today);
          if (!hasFutureEpisodes) {
            shouldMonitorSeason = false;
            // Update the season to not be monitored
            if (dbSeason) {
              TVSeriesModel.updateSeasonMonitored(dbSeason.id, false);
            }
          }
        }
        ...
        logger.error(`Failed to fetch season ${season.season_number}:`, error);
      }
    }

    logger.info(`Fetched ${totalEpisodes} episodes from TMDB for: ${series.title}`);
   ...
   */
  private async scanEpisodeFiles(seriesId: string, folderPath: string, seriesTitle: string): Promise<void> {
    try {
      logger.info(`Auto-scanning episode files for: ${seriesTitle}`);
      ...
      logger.info(`Auto-matched ${matchedCount} episode files for: ${seriesTitle}`);
    } catch (error) {
      logger.error(`Failed to auto-scan episodes for ${seriesTitle}:`, error);
    }
  }
   ...
   */
  private findAllVideoFiles(dir: string): Array<{ path: string; filename: string; size: number }> {
   ...
   */
  private parseEpisodeFilename(filename: string): { season?: number; episode?: number; quality?: string } {
    // Try S01E01 format
  ...
  private extractQuality(filename: string): string {
    ...
    if (upperFilename.includes('1080P')) return '1080p';
    if (upperFilename.includes('720P')) return '720p';
    if (upperFilename.includes('480P')) return '480p';
    
    if (upperFilename.includes('BLURAY') || upperFilename.includes('BLU-RAY')) return 'BluRay';
    if (upperFilename.includes('WEBDL') || upperFilename.includes('WEB-DL')) return 'WEBDL';
    if (upperFilename.includes('WEBRIP')) return 'WEBRip';
    if (upperFilename.includes('HDTV')) return 'HDTV';
   ...
   */
  async updateEpisodes(seriesId: string): Promise<number> {
    try {
      const series = TVSeriesModel.findById(seriesId);
      ...
      logger.info(`Updating episodes for series: ${series.title}`);
      ...
      for (const season of seasons) {
        try {
          ...
          const existingEpisodes = TVSeriesModel.findEpisodesBySeason(seriesId, season.season_number);
          ...
          logger.error(`Failed to update season ${season.season_number}:`, error);
        }
      }

      if (newEpisodes > 0) {
        logger.info(`Added ${newEpisodes} new episodes for: ${series.title}`);
      }

      return newEpisodes;
    } catch (error) {
      logger.error('Failed to update episodes:', error);
      return 0;
    }
  }
}

export const episodeFetcherService = new EpisodeFetcherService();
